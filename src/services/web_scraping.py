...
def _build_news(
        titles: List[str], urls: List[str], imgs: List[str], owner: str, date: str, category: str
        ) -> List[cntnt.News]:
    news = []

    for i in range(len(urls)):

        news.append(cntnt.News(-1, owner, titles[i], imgs[i], urls[i], "", -1, -1, date, category))

    return news


def add_new_container(news: List[cntnt.News]) -> List[cntnt.News]:
    # Lista de noticias
...
def split_by_container(news: List[cntnt.News]) -> Dict[int, List[cntnt.News]]:
    containers: Dict[int, List[cntnt.News]] = defaultdict(list)
    for new in news:
...
# --- MAKE'S ---
def _make_antena3news(structure: BeautifulSoup, category: str, date: str) -> List[cntnt.News]:
    ...
    return _build_news(
        titles=titles, urls=link_news, imgs=url_imgs, owner='antena3noticias', date=date, category=category
    )


def _make_lasexta_marca_news(structure: BeautifulSoup, category: str, date: str, owner: str) -> List[cntnt.News]:
...
def _make_nytimesnews(structure: BeautifulSoup, category: str, date: str) -> List[cntnt.News]:
    ...
    return [
        cntnt.News(-1, "The New York Times", titles[i][0], 'static\\img\\nytimes.png', link_news[i], "", -1, -1, date, category)
        for i in range(len(link_news))
    ]


# --- CATEGORIES ---
def _category_antena3(structure: BeautifulSoup) -> List[cntnt.News]:
...
def _category_lasexta(structure: BeautifulSoup) -> List[cntnt.News]:
...
def _category_marca(structure: BeautifulSoup) -> List[cntnt.News]:
...
def _category_nytimes(structure: BeautifulSoup) -> List[cntnt.News]:
...
# --- GETS ---
def get_nytimes() -> List[cntnt.News]:
...
def get_antena3() -> List[cntnt.News]:
    antena3_structure = BeautifulSoup(requests.get("https://www.antena3.com/noticias/").text, 'lxml')

    return _category_antena3(antena3_structure) + _make_antena3news(
        antena3_structure, "general", datetime.now().strftime(f'%Y-%m-%d')
    )


def get_lasexta_marca() -> List[cntnt.News]:
...
def get_containers(news: List[cntnt.News]) -> Dict[int, List[cntnt.News]]:
...
def add_content(news: List[cntnt.News]):
